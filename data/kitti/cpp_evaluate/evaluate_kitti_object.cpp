...
bool eval(const std::string& gt_dir, const std::string& result_dir, const std::vector<std::string>& file_list){
  ...
  for (std::size_t i=0; i<file_list.size(); i++) {
    ...
    // check for errors
    if (!gt_success) {
      std::cout << "ERROR: Couldn't read: " <<  file_name << " of ground truth.\n";
      return false;
    }
    if (!det_success) {
      std::cout << "ERROR: Couldn't read: " << file_name;
      return false;
    }
  }
  std::cout << "Done\n";
  ...
  if(eval_car){
    ...
    if(   !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[0],aos[0],EASY)
      ...
      std::cout << "ERROR: Car evaluation failed.\n";
      return false;
    }
    fclose(fp_det);
  ...
  if(eval_pedestrian){
    ...
    if(   !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[0],aos[0],EASY)
      ...
      std::cout << "ERROR: Pedestrian evaluation failed.\n";
      return false;
    }
    fclose(fp_det);
  ...
  if(eval_cyclist){
    ...
    if(   !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[0],aos[0],EASY)
      ...
      std::cout << "ERROR: Cyclist evaluation failed.\n";
      return false;
    }
    fclose(fp_det);
...
int main (int argc,char *argv[]) {
  ...
  // we need 3
  if (argc!=4) {
    cout << "Usage: ./eval_detection gt_folder result_folder split_file" << endl;
    return 1;
  }
  ...
  std::cout << "gt_folder = " << gt_folder << "\n";
  std::cout << "result_folder = " << result_folder << "\n";
  std::cout << "split_file = " << split_file << "\n";

  std::vector<std::string> filelist = loadSplits(split_file);
  std::cout << "Evaluating on " << filelist.size() << " Images" << std::endl;
