...
//  CUDA_CHECK(cudaBindTextureToArray( texRef, in_array));
//
//  dim3 block(16, 16, 1);
...
//
//    cudaArray *in_array;
//    CUDA_CHECK(cudaGraphicsMapResources(1, &cuda_gl_resource_, 0));
//    CUDA_CHECK(cudaGraphicsSubResourceGetMappedArray(&in_array, cuda_gl_resource_, 0, 0));
//    copyTexture(image_top_data + top[0]->offset(i, 0), in_array, width, height);
//    CUDA_CHECK(cudaGraphicsUnmapResources(1, &cuda_gl_resource_, 0));
//  }
//  viewer_->doneCurrent();
//}
//
