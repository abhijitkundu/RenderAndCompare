/**
 * @file H5EigenSparse.h
 * @brief H5EigenSparse
 *
 * @author Abhijit Kundu
 */

#ifndef _H5EIGEN_SPARSE_H_
#define _H5EIGEN_SPARSE_H_
...
class MyTriplet : public Eigen::Triplet<Scalar> {
 public:
  ...
  MyTriplet(const unsigned int &i, const unsigned int &j, const Scalar &v =
                Scalar(0))
...
class SparseH5Type : public H5::CompType {
 public:
  SparseH5Type(void)
    ...
    const H5::DataType * const datatypei =
        DatatypeSpecialization<unsigned int>::get();
    const H5::DataType * const datatype = DatatypeSpecialization<T>::get();
    assert(datatype->getSize() == sizeof(T));
    this->insertMember(std::string("r"), MyTriplet<T>::offsetof_row(),
                       *datatypei);
    this->insertMember(std::string("c"), MyTriplet<T>::offsetof_col(),
                       *datatypei);
    this->insertMember(std::string("v"), MyTriplet<T>::offsetof_value(),
                       *datatype);
    this->pack();
  }
...
void save_sparse(H5::CommonFG &h5group, const std::string &name,
...
template<typename SparseMatrixType>
void load_sparse(const H5::CommonFG &h5group, const std::string &name,
  ...
  if (ndims != 1) {
    throw std::runtime_error(
        "HDF5 array has incorrect number of dimensions to represent a sparse matrix.");
  }
  Eigen::Matrix<typename SparseMatrixType::Index, 2, 1> shape;
