...
void compute_seg_histograms(const uint8_t* const gt_image,
  ...
  CHECK_CUDA(cudaMalloc(&d_gt_image, image_bytes));
  CHECK_CUDA(cudaMalloc(&d_pred_image, image_bytes));
  CHECK_CUDA(cudaMemcpy(d_gt_image, gt_image, image_bytes, cudaMemcpyHostToDevice));
  CHECK_CUDA(cudaMemcpy(d_pred_image, pred_image, image_bytes, cudaMemcpyHostToDevice));
...
//  float2 mean_iou = d_mean_ious[0];
  ...
  CHECK_CUDA(cudaFree((void*)d_gt_image));
  CHECK_CUDA(cudaFree((void*)d_pred_image));
}
...
void compute_confusion_matrix(const uint8_t* const gt_image,
  ...
  CHECK_CUDA(cudaMalloc((void**)(&d_conf_mat), conf_mat_bytes));
  CHECK_CUDA(cudaMemset(d_conf_mat, 0, conf_mat_bytes));
  ...
  CHECK_CUDA(cudaMemcpy(h_conf_mat.data(), d_conf_mat, h_conf_mat.size() * sizeof(CMScalar), cudaMemcpyDeviceToHost));
...
void compute_confusion_tensor(const uint8_t* const gt_image,
  ...
  CHECK_CUDA(cudaMemset(d_conf_mat_ptr, 0, conf_mat_bytes));
...
//  assert(cudaStreamSynchronize(gpu_device.stream()) == cudaSuccess);
  ...
  CHECK_CUDA(cudaMemcpy(h_intersection_hist.data(), d_intersection_hist_ptr, hist_bytes, cudaMemcpyDeviceToHost));
  CHECK_CUDA(cudaMemcpy(h_union_hist.data(), d_union_hist_ptr, hist_bytes, cudaMemcpyDeviceToHost));
...
void compute_cmat_warped_iou(const uint8_t* const gt_image, const uint8_t* const pred_image, int width, int height) {
  ...
  CHECK_CUDA(cudaMalloc(&d_conf_mat_ptr, conf_mat_bytes));
  CHECK_CUDA(cudaMemset(d_conf_mat_ptr, 0, conf_mat_bytes));
  ...
  CHECK_CUDA(cudaFree((void*)d_conf_mat_ptr));
  CHECK_CUDA(cudaFree((void*)d_gt_image));
  CHECK_CUDA(cudaFree((void*)d_pred_image));
}
...
void computeHistogramWithAtomics(const uint8_t* const image, int width, int height, int *hist, int num_labels) {
  ...
  ImageScalar* d_image;
  const std::size_t image_bytes = width * height * sizeof(ImageScalar);
  CHECK_CUDA(cudaMalloc(&d_image, image_bytes));
  CHECK_CUDA(cudaMemcpy(d_image, image, image_bytes, cudaMemcpyHostToDevice));

  HistScalar *d_hist;
  CHECK_CUDA(cudaMalloc(&d_hist, num_labels * sizeof(HistScalar)));

  using CuteGL::GpuTimer;
  GpuTimer gpu_timer;
  gpu_timer.start();

  CHECK_CUDA(cudaMemset(d_hist, 0, num_labels * sizeof(HistScalar)));
  ...
  CHECK_CUDA(cudaMemcpy(hist, d_hist, num_labels * sizeof(HistScalar), cudaMemcpyDeviceToHost));

  CHECK_CUDA(cudaFree((void*)d_image));
  CHECK_CUDA(cudaFree((void*)d_hist));
}
...
void computeHistogramWithSharedAtomics(const uint8_t* const image, int width, int height, int *hist, int num_labels) {
  ...
  CHECK_CUDA(cudaMemcpy(hist, d_hist, NumLabels * sizeof(HistScalar), cudaMemcpyDeviceToHost));

  CHECK_CUDA(cudaFree((void*)d_image));
  CHECK_CUDA(cudaFree((void*)d_hist));
  CHECK_CUDA(cudaFree((void*)d_part_hist));
}
...
void computeHistogramWithThrust(const uint8_t* const image, int width, int height, int *hist, int num_bins) {
  ...
  CHECK_CUDA(cudaMemcpy(d_image.data().get(), image, width * height * sizeof(ImageScalar), cudaMemcpyHostToDevice));
...
//  HistScalar num_bins = d_image.back() + 1;
  ...
  CHECK_CUDA(cudaMemcpy(hist, histogram.data().get(), num_bins * sizeof(HistScalar), cudaMemcpyDeviceToHost));
}
...
void computeIoUseq(const Eigen::Tensor<uint8_t, 4, Eigen::RowMajor>& gt_images,
  ...
  CHECK_CUDA(cudaMalloc((void**)(&d_gt_images), gt_images_bytes));
  CHECK_CUDA(cudaMalloc((void**)(&d_pred_images), pred_images_bytes));
  CHECK_CUDA(cudaMemcpy(d_gt_images, gt_images.data(), gt_images_bytes, cudaMemcpyHostToDevice));
  CHECK_CUDA(cudaMemcpy(d_pred_images, pred_images.data(), pred_images_bytes, cudaMemcpyHostToDevice));
  ...
  CHECK_CUDA(cudaFree((void*)d_gt_images));
  CHECK_CUDA(cudaFree((void*)d_pred_images));
...
void computeIoUwithCUDApar(const Eigen::Tensor<uint8_t, 4, Eigen::RowMajor>& gt_images,
  ...
  for (int trial = 0; trial < trials; ++trial) {
    ...
    CHECK_CUDA(cudaMalloc((void**)(&d_hist), d_hist_bytes));
    CHECK_CUDA(cudaMemset(d_hist, 0, d_hist_bytes));
...
void computeIoUStreams(const Eigen::Tensor<uint8_t, 4, Eigen::RowMajor>& gt_images,
                       const Eigen::Tensor<uint8_t, 4, Eigen::RowMajor>& pred_images, const int trials) {
  using ImageScalar = uint8_t;
  using CMScalar = int;
  ...
  for (int trial = 0; trial < trials; ++trial) {
    ...
    std::vector<cudaStream_t> streams(images_per_blob);
    for (Eigen::Index i = 0; i < images_per_blob; ++i) {
      ...
      CMScalar* d_conf_mat_ptr = d_conf_mats.data().get() + NumLabels * NumLabels * i;
      {
        ...
        assert(blocksize >= NumLabels * NumLabels);
        const Eigen::Index image_data_offset = image_size * i;
        confusion_matrix_shared_1d_strided_atomics<NumLabels><<<gridsize, blocksize, NumLabels * NumLabels* sizeof(CMScalar), streams[i]>>>(d_gt_images + image_data_offset,
            d_pred_images + image_data_offset ,
            image_size, d_conf_mat_ptr);
      }
      {
        dim3 blockdim(32, 32);
        warp_iou_kernel<NumLabels><<<1, blockdim, 32 * 3 * sizeof(CMScalar), streams[i]>>>(d_conf_mat_ptr, mean_ious.data().get() + i);
      }
