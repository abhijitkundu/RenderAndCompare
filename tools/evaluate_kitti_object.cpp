...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,
  ...
  for (size_t i=0; i<groundtruth.size(); i++){
    ...
    for(size_t j=0; j<pr_tmp.v.size(); j++)
...
std::vector<std::string> loadSplits(const std::string& split_file_path) {
  std::vector<std::string> filenames;

  std::ifstream split_file(split_file_path);

  std::copy(std::istream_iterator<std::string>(split_file),
            std::istream_iterator<std::string>(), std::back_inserter(filenames));
...
bool eval(const string& gt_dir, const string& result_dir, const vector<string>& file_list){
  ...
  run_system_command(("mkdir " + plot_dir).c_str());
  ...
  for (size_t i=0; i < file_list.size(); ++i) {
    ...
    // file name
    const string& file_name = file_list[i];
    ...
    // check for errors
    if (!gt_success) {
      cout << "ERROR: Couldn't read: " <<  file_name << " of ground truth.\n";
      return false;
    }
    if (!det_success) {
      cout << "ERROR: Couldn't read: " << file_name << " of result.\n";
      return false;
    }
  }
  cout << "Done\n";
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      cout << "Starting 2D evaluation (" << CLASS_NAMES[c] << ")\n";
      ...
      if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, imageBoxOverlap, precision[0], aos[0], EASY, IMAGE)
        ...
        cout <<  CLASS_NAMES[c] << "evaluation failed.\n";
        return false;
      }
      fclose(fp_det);
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_3d[c]) {
      cout << "Starting 3D evaluation (" << CLASS_NAMES[c] << ") ...\n";
...
int main(int argc, char **argv) {
  ...
  // we need 3 arguments
  if (argc!=4) {
    cout << "Usage: " << argv[0] << " gt_folder result_folder split_file" << endl;
    return EXIT_FAILURE;
  }

  // read arguments
  const string gt_folder = argv[1];
  const string result_folder = argv[2];
  const string split_file = argv[3];

  cout << "gt_folder = " << gt_folder << "\n";
  cout << "result_folder = " << result_folder << "\n";
  cout << "split_file = " << split_file << "\n";

  vector<string> filelist = loadSplits(split_file);
  cout << "Evaluating on " << filelist.size() << " Images" << endl;

  // run evaluation
  if (eval(gt_folder, result_folder, filelist))
