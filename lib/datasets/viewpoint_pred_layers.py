...
class ViewpoinPredictionDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        top[0].reshape(self.batch_size, 3, params.im_size[1], params.im_size[0])
        top[1].reshape(self.batch_size,)


        image_files, self.labels = self.get_image_files_and_labels(params)
        assert len(image_files) == len(self.labels), 'Numbers of image files ({}) do not match number of labels ({})'.format(len(image_files), len(self.labels))
        assert len(image_files) >= self.batch_size, 'Numbers of image files ({}) should be >= batch size ({})'.format(len(image_files), len(self.batch_size))
    ...
    def get_image_files_and_labels(self, params):
        ...
        assert osp.exists(split_file), 'Path does not exist: {}'.format(split_file)
        ...
        with open(split_file, 'rb') as f:
            for line in f:
                ...
                assert azimuth_label < azimuth_bins, "label (%d) >= num_classes (%d) for Image at %s" % (azimuth_label, azimuth_bins, osp.basename(filename))
